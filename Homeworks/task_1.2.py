...
# Второй вариант кода:
my_favorite_songs = [['Waste a Moment', 3.03], ['New Salvation', 4.02], ['Staying\' Alive', 3.40], ['Out of Touch', 3.03], ['A Sorta Fairytale', 5.28], ['Easy', 4.15], ['Beautiful Day', 4.04], ['Nowhere to Run', 2.58], ['In This World', 4.02] ]
#print([x[1] for x in my_favorite_songs])
#print(type(my_favorite_songs))

from random import sample
time = sample([x[1] for x in my_favorite_songs], 3)
print(time)
time = str(sum(time))
print(time)
print(type(time))
...
import datetime
time_format = datetime.datetime.strptime(time, "%M.%S")
print("Три песни звучат:", time_format.time(), "минут")
